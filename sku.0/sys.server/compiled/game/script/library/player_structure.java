...
public class player_structure extends script.base_script
{
    ...
    public static void finalizePackUp(obj_id player, obj_id structure) throws InterruptedException
    {
        ...
        {
            CustomerServiceLog("housepackup", "Structure (" + structure + ") was abandoned and was not being packed by Owner (" + getPlayerName(structure_owner) + ") " + structure_owner + ". A player ( " + getPlayerName(player) + " is packing this structure. Sending job to the Abandoned Structure Pack Up function.");
        ...
        {
            CustomerServiceLog("housepackup", "Structure (" + structure + ") could not be packed by player/owner (" + getPlayerName(player) + ") OID (" + player + ") The cause: Player/owner -" + " isLoaded = " + player.isLoaded() + " isAuthoritative = " + player.isAuthoritative());
            return;
        }
        CustomerServiceLog("housepackup", "Player/Owner " + getPlayerName(player) + " OID (" + player + ") is attempting to pack structure (" + structure + ",abandoned=" + isAbandoned + ") Structure is loaded and is Authoritative.");
        if (!structure.isLoaded() || !structure.isAuthoritative())
        {
            ...
            CustomerServiceLog("housepackup", "Structure (" + structure + ") could not be packed by player/owner (" + getPlayerName(player) + ") OID (" + player + ") because structure -" + " isLoaded = " + structure.isLoaded() + " isAuthoritative = " + structure.isAuthoritative());
            return;
        }
    ...
    public static void finalizeAbandonedStructurePackUp(obj_id player, obj_id structure) throws InterruptedException
    {
        ...
        if (!structure.isLoaded() || !structure.isAuthoritative())
        {
            ...
            CustomerServiceLog("housepackup", "Structure (" + structure + ") could not be packed by player (" + getPlayerName(player) + ") structure -" + " isLoaded = " + structure.isLoaded() + " isAuthoritative = " + structure.isAuthoritative());
            return;
        }
        obj_id datapad = utils.getPlayerDatapad(player);
        if (!isIdValid(datapad))
    ...
    public static void finalizeCityPackUp(obj_id player, obj_id structure) throws InterruptedException
    {
        ...
        if (!cityExists(city_id))
        {
            ...
            CustomerServiceLog("city_house_packup", "City House Packup was called on Structure(" + structure + ") by player(" + player + ") but house is no longer in a City - Removing isCityAbandoned Flag.");
            removeCityAbandoned(structure);
            return;
        }
        obj_id structure_owner = getOwner(structure);
        CustomerServiceLog("city_house_packup", "Player " + getPlayerName(player) + " (" + player + ") is attempting to pack structure (" + structure + ", City Abandoned = " + isCityAbandoned(structure) + ") owned by player " + getPlayerName(structure_owner) + " (" + structure_owner + ")");
        obj_id datapad = utils.getPlayerDatapad(player);
        if (!isIdValid(datapad))
        {
        ...
        if (!player_structure.isCityPackupAuthoritative(player, structure))
        {
            ...
            CustomerServiceLog("city_house_packup", "'finalizeCityPackUp' - Structure (" + structure + ") could not be packed by player (" + getPlayerName(player) + ") player -" + " isLoaded(Player) = " + player.isLoaded() + " isAuthoritative(Player) = " + player.isAuthoritative() + " isLoaded(Structure) = " + structure.isLoaded() + " isAuthoritative(Structure) = " + structure.isAuthoritative());
            return;
        }
        location where = getLocation(structure);
        CustomerServiceLog("city_house_packup", "Player " + getPlayerName(player) + " (" + player + ") is ABOUT to pack abandoned structure (" + structure + ",abandoned=" + isCityAbandoned(structure) + "," + where.toString() + ") owned by player " + getPlayerName(structure_owner) + " (" + structure_owner + ")");
        if (cityExists(city_id))
        {
    ...
    public static String[] getStorageTypeNamesArray(obj_id structure) throws InterruptedException
    {
        ...
        {
            _storageIncreaseTypes = new String[storageIncreaseTypes.size()];
            storageIncreaseTypes.toArray(_storageIncreaseTypes);
        }
        return _storageIncreaseTypes;
    }
