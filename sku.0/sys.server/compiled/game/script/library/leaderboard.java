...
public class leaderboard extends script.base_script
{
     ...
     *
     * @param player
     *         obj_id of player to add
     * @param type
     *         = LEADERBOARD_TYPE_FISHING or LEADERBOARD_TYPE_GCW
     */
    ...
    /**
     * @param isCity
     *         true if a city, false if a guild (to avoid 2 methods)
     ...
     *         = LEADERBOARD_TYPE_FISHING or LEADERBOARD_TYPE_GCW
     * @param faction
     *         = FACTION_HASH_IMPERIAL or FACTION_HASH_REBEL
     * @param period
     *         = LEADERBOARD_PERIOD_CURRENT, LEADERBOARD_PERIOD_PREVIOUS, or LEADERBOARD_PERIOD_2WEEKSAGO
    ...
    /**
     * @param isCity
     *         true if asking for city data, false if asking for guild Data
    ...
    public static void handleGcwLeaderboardPlayerWinners(int faction, obj_id[] players, float[] scores) throws InterruptedException
    {
        ...
        for (int i = 0; i < players.length; i++)
        {
            ...
            { // so we don't add null entries popped from leaderboard object array copy
    ...
    public static void handleGcwLeaderboardGuildWinners(int faction, int[] guildIds, float[] scores) throws InterruptedException
    {
        ...
        {
            if (isIdValid(guildGetLeader(guildIds[i])))
            { // so we don't add null entries popped from leaderboard object array copy
    ...
    public static void handleGcwLeaderboardCityWinners(int faction, int[] cityIds, float[] scores) throws InterruptedException
    {
        ...
        {
            if (isIdValid(cityGetLeader(cityIds[i])))
            { // so we don't add null entries popped from leaderboard object array copy
    ...
    public static void renderGcwLeaderboardDataToPlayer(obj_id player, obj_id requestingObject, int renderType) throws InterruptedException
    {
        ...
        {
            if (usedCachedData && hasCachedLocalScore(requestingObject, renderType))
            { // if we should use cached data & we have it
                score = getCachedLocalScore(requestingObject, renderType);
            }
            else
            { // actually pull the data and cache it
        ...
        if (usedCachedData && hasCachedScoreData(requestingObject, renderType))
        { // if we should use cached data & we have it
        ...
        {
            broadcast(player, "There is no score data available for the requested GCW Leaderboard. Please try again later.");
            return;
        }
    ...
    public static float getPlayerRelativeScoreToTransposedLeaderboardScores(obj_id player, int faction, int period)
    {
        ...
        { // don't calculate a relative score for someone already ranked because that wouldn't make sense
    ...
    public static void renderGcwLeaderboardEntityHistory(obj_id player, obj_id requestingObject, boolean isCity) throws InterruptedException
    {
        ...
        {
            broadcast(player, "There is no GCW Leaderboard History available for your " + type + " at this time.");
            return;
        }
        boolean useCachedData = getCurrentLeaderboardPeriodStartTime() <= getPlayerLastLoginTime(player);
     ...
     *
     * @param isCity
     *         true if it's a city, false if it's a guild (just to save having to make 2 methods)
     */
     ...
     */
    public static void announceGcwLeaderboardWinnersToDiscord(int winnerType, int faction, obj_id[] players, int[] entities, float[] scores) throws InterruptedException
    {
        if (ANNOUNCEMENT_WEBHOOK_URL == null || ANNOUNCEMENT_WEBHOOK_URL.isEmpty())
        {
            return;
    ...
    public static void handleGcwLeaderboardDataRequestCsInternalOnly(obj_id player, int menuOptionIndex) throws InterruptedException
    {
        ...
        switch (menuOptionIndex)
        {
            ...
            case 0:
                broadcast(player, "Nothing is implemented for this option yet because Aconite couldn't decide what all would be useful here. If you have ideas, feel free to message him.");
                break;
            case 1:
                messageTo(getMasterLeaderboardObject(), "handleCurrentPeriodUpdateHeartbeat", null, 0f, false);
                broadcast(player, "Requesting a forced leaderboard period update...");
            ...
            case 2:
                ...
                broadcast(player, "Requesting a forced leaderboard period reset...");
