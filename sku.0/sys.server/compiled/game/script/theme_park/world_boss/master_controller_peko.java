...
public class master_controller_peko extends script.base_script
{
    ...
    public int OnAttach(obj_id self) throws InterruptedException
    {
        ...
        or_player.doWorldBossAnnounce(self, or_player.WORLD_BOSS_PEKO);
    ...
    public int OnIncapacitated(obj_id self, obj_id killer) throws InterruptedException
    {
        ...
        or_player.doWorldBossDeathMsg(self, killer); //only thing that is needed.
        return SCRIPT_CONTINUE;
    }
    ...
    public int OnCreatureDamaged(obj_id self, obj_id attacker, obj_id wpn, int[] damage) throws InterruptedException
    {
        ...
        if (percentHealth <= 75)
        {
            ...
            {
                or_player.createCircleSpawn(self, self, "peko_peko", 12, 24);
        ...
        if (percentHealth <= 10)
        {
            if (!utils.hasScriptVar(self, "hasLastStand"))
            {
                ...
                or_player.createCircleSpawn(self, self, "peko_peko_albatross_high", 4, 24);
                staggerPlayers(self, players);
                for (obj_id who : players)
                {
    ...
    {
        float MAX_DISTANCE = or_player.WORLD_BOSS_CREDIT_RANGE + 12f; //bring them outside the credit range, so if they are drags, they will need to manually run back.
