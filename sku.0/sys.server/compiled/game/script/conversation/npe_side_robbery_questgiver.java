...
public class npe_side_robbery_questgiver extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (npe_side_robbery_questgiver_condition_playerDoneWithQuest(player, self))
        {
            npe_side_robbery_questgiver_action_facePlayer(player, self);
            string_id message = new string_id(c_stringFile, "s_23");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (npe_side_robbery_questgiver_condition_playerOnQuest(player, self))
        {
            npe_side_robbery_questgiver_action_facePlayer(player, self);
            ...
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (npe_side_robbery_questgiver_condition_playerHasPrototype(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            boolean hasResponse1 = false;
            if (npe_side_robbery_questgiver_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse1 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "npe_side_robbery_questgiver", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 2 && npe_side_robbery_questgiver_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 5 && npe_side_robbery_questgiver_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && npe_side_robbery_questgiver_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && npe_side_robbery_questgiver_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
