...
public class itp_vader_mara_jade extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (itp_vader_mara_jade_condition_vaderTaskActive(player, self))
        {
            itp_vader_mara_jade_action_vaderSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_32");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (itp_vader_mara_jade_condition__defaultCondition(player, self))
        {
