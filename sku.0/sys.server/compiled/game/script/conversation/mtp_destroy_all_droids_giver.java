...
public class mtp_destroy_all_droids_giver extends script.base_script
{
    ...
    public boolean mtp_destroy_all_droids_giver_condition_alreadyHasQuest(obj_id player, obj_id npc) throws InterruptedException
    {
        return groundquests.isQuestActive(player, "mtp_hideout_instance_kill_all_droids");
    }
    public boolean mtp_destroy_all_droids_giver_condition_returningQuestSuccess(obj_id player, obj_id npc) throws InterruptedException
    {
        return groundquests.isQuestActive(player, "mtp_hideout_instance_kill_all_droids_success");
    }
    public boolean mtp_destroy_all_droids_giver_condition_notYetReadyForAnother(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public boolean mtp_destroy_all_droids_giver_condition_returningQuestFailure(obj_id player, obj_id npc) throws InterruptedException
    {
        return groundquests.isQuestActive(player, "mtp_hideout_instance_kill_all_droids_fail");
    }
    public void mtp_destroy_all_droids_giver_action_grantDroidQuest(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public void mtp_destroy_all_droids_giver_action_sendDroidSuccessSignal(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public void mtp_destroy_all_droids_giver_action_sendDroidFailureSignal(obj_id player, obj_id npc) throws InterruptedException
    {
        groundquests.sendSignal(player, "mtp_kill_all_droids_failed");
    }
    public int mtp_destroy_all_droids_giver_handleBranch5(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (mtp_destroy_all_droids_giver_condition_returningQuestSuccess(player, self))
        {
            mtp_destroy_all_droids_giver_action_sendDroidSuccessSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_4");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_destroy_all_droids_giver_condition_returningQuestFailure(player, self))
        {
            mtp_destroy_all_droids_giver_action_sendDroidFailureSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_19");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_destroy_all_droids_giver_condition_alreadyHasQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_7");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_destroy_all_droids_giver_condition_notYetReadyForAnother(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_9");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_destroy_all_droids_giver_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "mtp_destroy_all_droids_giver", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    {
        if (!conversationId.equals("mtp_destroy_all_droids_giver"))
        {
            return SCRIPT_CONTINUE;
        }
        int branchId = utils.getIntScriptVar(player, "conversation.mtp_destroy_all_droids_giver.branchId");
        if (branchId == 5 && mtp_destroy_all_droids_giver_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
