...
public class dagorel extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (dagorel_condition_finishedBorvosGuardQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_5");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (dagorel_condition_onBorvosGuardQuest(player, self))
        {
            dagorel_action_broughtMeMedicine(player, self);
            string_id message = new string_id(c_stringFile, "s_6");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (dagorel_condition__defaultCondition(player, self))
        {
