...
public class corellia_39_jaalib_rigel extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (corellia_39_jaalib_rigel_condition_taskOneComplete(player, self))
        {
            doAnimationAction(self, "nod_head_multiple");
            corellia_39_jaalib_rigel_action_taskOneCompletedSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_48");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (corellia_39_jaalib_rigel_condition_taskOneActive(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_49");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (corellia_39_jaalib_rigel_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "corellia_39_jaalib_rigel", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 3 && corellia_39_jaalib_rigel_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
