...
public class prof_officer_21 extends script.base_script
{
    ...
    public int prof_officer_21_handleBranch5(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        ...
        {
            if (prof_officer_21_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.prof_officer_21.branchId", 6);
        ...
        if (response.equals("s_26"))
        {
            if (prof_officer_21_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.prof_officer_21.branchId", 9);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
    ...
    public int prof_officer_21_handleBranch6(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("s_18"))
        {
            if (prof_officer_21_condition__defaultCondition(player, npc))
            {
                prof_officer_21_action_grantOfficerQuest(player, npc);
        ...
        if (response.equals("s_30"))
        {
            if (prof_officer_21_condition__defaultCondition(player, npc))
            {
                ...
                if (hasResponse)
                {
                    ...
                    utils.setScriptVar(player, "conversation.prof_officer_21.branchId", 10);
                    npcSpeak(player, message);
                    npcSetConversationResponses(player, responses);
                }
                else 
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (prof_officer_21_condition_doneWithOfficerQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_6");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (prof_officer_21_condition_notOfficer(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_5");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (prof_officer_21_condition_finishingOfficer(player, self))
        {
            prof_officer_21_action_sendFinishSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_8");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (prof_officer_21_condition_isDoingOfficerQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_10");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (prof_officer_21_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "prof_officer_21", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        int branchId = utils.getIntScriptVar(player, "conversation.prof_officer_21.branchId");
        if (branchId == 5 && prof_officer_21_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && prof_officer_21_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && prof_officer_21_handleBranch9(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 10 && prof_officer_21_handleBranch10(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
