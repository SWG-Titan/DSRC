...
public class naboo_theed_toff_henrou extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (naboo_theed_toff_henrou_condition_completedSkaakQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_37");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (naboo_theed_toff_henrou_condition_reGiveHugoPointer(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "naboo_theed_toff_henrou", message, responses);
            }
            else 
            {
        ...
        if (naboo_theed_toff_henrou_condition_lastStepSkaakQuest(player, self))
        {
            naboo_theed_toff_henrou_action_signal_skaak_05(player, self);
            string_id message = new string_id(c_stringFile, "s_73");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (naboo_theed_toff_henrou_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
        ...
        if (naboo_theed_toff_henrou_condition_onSkaakQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_36");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (naboo_theed_toff_henrou_condition_startSkaakQuest(player, self))
        {
            naboo_theed_toff_henrou_action_endGotoToff(player, self);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && naboo_theed_toff_henrou_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && naboo_theed_toff_henrou_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && naboo_theed_toff_henrou_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && naboo_theed_toff_henrou_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
