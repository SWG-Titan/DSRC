...
public class corellia_coronet_daclif_gallamby extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (corellia_coronet_daclif_gallamby_condition_thankfulDiktatComplete(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_4");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (corellia_coronet_daclif_gallamby_condition_thankfulDiktatActive(player, self))
        {
            ...
            if (corellia_coronet_daclif_gallamby_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "corellia_coronet_daclif_gallamby", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
        ...
        if (corellia_coronet_daclif_gallamby_condition_diktatSearchComplete(player, self))
        {
        ...
        if (corellia_coronet_daclif_gallamby_condition_diktatSearchActive(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_14");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (corellia_coronet_daclif_gallamby_condition_presentSearchWrits(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 2 && corellia_coronet_daclif_gallamby_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && corellia_coronet_daclif_gallamby_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && corellia_coronet_daclif_gallamby_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
