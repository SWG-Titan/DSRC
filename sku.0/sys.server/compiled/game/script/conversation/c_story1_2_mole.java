...
public class c_story1_2_mole extends script.base_script
{
    ...
    public boolean c_story1_2_mole_condition_atKailaStep(obj_id player, obj_id npc) throws InterruptedException
    {
        ...
        return questIsTaskActive(questId1, kaila1, player) || questIsTaskActive(questId2, kaila2, player) || questIsTaskActive(questId3, kaila3, player);
    }
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (c_story1_2_mole_condition__defaultCondition(player, self))
        {
            c_story1_2_mole_action_facePlayer(player, self);
            string_id message = new string_id(c_stringFile, "s_53");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (c_story1_2_mole_condition_atKailaStep(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "c_story1_2_mole", message, responses);
            }
            else 
            {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && c_story1_2_mole_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
