...
public class u13_boba_fett extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (u13_boba_fett_condition_returning_sidequest(player, self))
        {
            u13_boba_fett_action_signal_u13_vengeance_sidequest(player, self);
            string_id message = new string_id(c_stringFile, "s_9");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (u13_boba_fett_condition_sidequest_active(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_8");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (u13_boba_fett_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "u13_boba_fett", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 3 && u13_boba_fett_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
