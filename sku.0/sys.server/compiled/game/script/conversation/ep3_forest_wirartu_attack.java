...
public class ep3_forest_wirartu_attack extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (ep3_forest_wirartu_attack_condition_isEpicTaskActiveOne(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_865");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (ep3_forest_wirartu_attack_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "ep3_forest_wirartu_attack", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 2 && ep3_forest_wirartu_attack_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
