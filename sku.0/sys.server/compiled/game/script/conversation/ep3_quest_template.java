...
public class ep3_quest_template extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (ep3_quest_template_condition_hasCompletedAllQuests(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_256");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (ep3_quest_template_condition_isOnQuestTwo(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_251");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (ep3_quest_template_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "ep3_quest_template", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
        ...
        if (!ep3_quest_template_condition_hasCompletedQuestTwo(player, self))
        {
        ...
        if (ep3_quest_template_condition_isOnQuestOne(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 2 && ep3_quest_template_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && ep3_quest_template_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && ep3_quest_template_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && ep3_quest_template_handleBranch9(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
