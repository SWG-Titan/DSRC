...
public class rtp_mon_mothma_main extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (rtp_mon_mothma_main_condition_notRebel(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_44");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (!rtp_mon_mothma_main_condition_completedAckbar(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_13");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (rtp_mon_mothma_main_condition_rebel_isOnLeave(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_43");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (rtp_mon_mothma_main_condition_rtp_mon_mothma_01_complete(player, self))
        {
            rtp_mon_mothma_main_action_rtp_mon_mothma_01_signal(player, self);
            string_id message = new string_id(c_stringFile, "s_20");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (rtp_mon_mothma_main_condition_rtp_mon_mothma_01_task02_active(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_14");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (rtp_mon_mothma_main_condition_rtp_mon_mothma_01_active(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_32");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (rtp_mon_mothma_main_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "rtp_mon_mothma_main", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 7 && rtp_mon_mothma_main_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
