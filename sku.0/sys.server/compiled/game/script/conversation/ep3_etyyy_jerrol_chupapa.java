...
public class ep3_etyyy_jerrol_chupapa extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (ep3_etyyy_jerrol_chupapa_condition__defaultCondition(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_285");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (ep3_etyyy_jerrol_chupapa_condition_doesNotHaveBrodyQuest(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "ep3_etyyy_jerrol_chupapa", message, responses);
            }
            else 
            {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && ep3_etyyy_jerrol_chupapa_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
