...
public class meatlump_vendor extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (meatlump_vendor_condition_canPurchase(player, self))
        {
            meatlump_vendor_action_showResourceVendorUI(player, self);
            string_id message = new string_id(c_stringFile, "s_4");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (meatlump_vendor_condition__defaultCondition(player, self))
        {
