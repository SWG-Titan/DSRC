...
public class bestine_artist02 extends base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (bestine_artist02_condition_hasFeaturedArt_EventActive(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_5b1aeaab");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (!bestine_artist02_condition_hasAlreadyVoted(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "bestine_artist02", message, responses);
            }
            else 
            {
        ...
        if (bestine_artist02_condition_isActiveInEvent(player, self))
        {
        ...
        if (bestine_artist02_condition_hasFeaturedArt(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_c062519");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (bestine_artist02_condition__defaultCondition(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && bestine_artist02_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 5 && bestine_artist02_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
