...
public class loveday_disillusion_pet_lover extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (loveday_disillusion_pet_lover_condition_petFound(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_16");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (loveday_disillusion_pet_lover_condition_petSearch(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_5");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (loveday_disillusion_pet_lover_condition_speakWithHim(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_7");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (loveday_disillusion_pet_lover_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "loveday_disillusion_pet_lover", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 3 && loveday_disillusion_pet_lover_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && loveday_disillusion_pet_lover_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
