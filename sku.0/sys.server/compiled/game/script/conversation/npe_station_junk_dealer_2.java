...
public class npe_station_junk_dealer_2 extends script.base_script
{
    ...
    public boolean npe_station_junk_dealer_2_condition_check_inv(obj_id player, obj_id npc) throws InterruptedException
    {
        return smuggler.checkInventory(player, npc);
    }
    public void npe_station_junk_dealer_2_action_start_dealing(obj_id player, obj_id npc) throws InterruptedException
    {
        dictionary params = new dictionary();
        params.put("player", player);
        messageTo(npc, "startDealing", params, 1.0f, false);
    }
    public void npe_station_junk_dealer_2_action_facePlayer(obj_id player, obj_id npc) throws InterruptedException
    {
        faceTo(npc, player);
    }
    public int npe_station_junk_dealer_2_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int npe_station_junk_dealer_2_handleBranch2(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (npe_station_junk_dealer_2_condition__defaultCondition(player, self))
        {
            npe_station_junk_dealer_2_action_facePlayer(player, self);
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "npe_station_junk_dealer_2", message, responses);
            }
            else 
            {
    ...
    {
        if (!conversationId.equals("npe_station_junk_dealer_2"))
        {
            return SCRIPT_CONTINUE;
        }
        int branchId = utils.getIntScriptVar(player, "conversation.npe_station_junk_dealer_2.branchId");
        if (branchId == 1 && npe_station_junk_dealer_2_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && npe_station_junk_dealer_2_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
