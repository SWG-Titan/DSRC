...
public class som_battlefield_miner_leader extends script.base_script
{
    ...
    {
        if (utils.hasScriptVar(self, "convoUnderway"))
        {
            broadcast(player, "The shift leader is already conversing with someone else.");
            return SCRIPT_CONTINUE;
        }
        else
        {
            utils.setScriptVar(self, "convoUnderway", player);
            messageTo(self, "unlockMob", null, 60f, false);
        }
        if (ai_lib.isInCombat(self) || ai_lib.isInCombat(player))
        {
            return SCRIPT_OVERRIDE;
        }
        if (som_battlefield_miner_leader_condition__defaultCondition(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_4");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (som_battlefield_miner_leader_condition_isNotFollowing(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (som_battlefield_miner_leader_condition_isMinerFollowing(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse1 = true;
            ...
            if (som_battlefield_miner_leader_condition_isNotDeployed(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse2 = true;
            ...
            if (som_battlefield_miner_leader_condition_isDeployed(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse3 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "som_battlefield_miner_leader", message, responses);
            }
            else 
            {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && som_battlefield_miner_leader_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
    ...
    public int unlockMob(obj_id npc, dictionary params) throws InterruptedException
    {
        utils.removeScriptVar(npc, "convoUnderway");
        return SCRIPT_CONTINUE;
    }
}
