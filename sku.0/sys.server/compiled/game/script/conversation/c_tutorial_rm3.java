...
public class c_tutorial_rm3 extends script.base_script
{
    ...
    public boolean c_tutorial_rm3_condition_playerOnQuest(obj_id player, obj_id npc) throws InterruptedException
    {
        ...
        return questIsQuestActive(questId, player) && !questIsTaskComplete(questId, explainQuestJournal, player) && questIsTaskComplete(questId, explainInventory, player);
    }
    public boolean c_tutorial_rm3_condition_TaskComplete(obj_id player, obj_id npc) throws InterruptedException
    {
        int questId = questGetQuestId("quest/c_newbie_hall_01");
        int explainQuestJournal = groundquests.getTaskId(questId, "explainQuestJournal");
        return questIsTaskComplete(questId, explainQuestJournal, player);
    }
    public boolean c_tutorial_rm3_condition_playerOnQuestStep02(obj_id player, obj_id npc) throws InterruptedException
    {
        int questId = questGetQuestId("quest/c_newbie_hall_01");
        int explainInventory = groundquests.getTaskId(questId, "explainInventory");
        return questIsQuestActive(questId, player) && !questIsTaskComplete(questId, explainInventory, player);
    }
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (c_tutorial_rm3_condition_playerOnQuest(player, self))
        {
            doAnimationAction(self, "nod_head_once");
            c_tutorial_rm3_action_facePlayer(player, self);
            string_id message = new string_id(c_stringFile, "s_21");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (c_tutorial_rm3_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "c_tutorial_rm3", message, responses);
            }
            else 
            {
        ...
        if (c_tutorial_rm3_condition_TaskComplete(player, self))
        {
        ...
        if (c_tutorial_rm3_condition_playerOnQuestStep02(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && c_tutorial_rm3_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && c_tutorial_rm3_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && c_tutorial_rm3_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 5 && c_tutorial_rm3_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && c_tutorial_rm3_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
