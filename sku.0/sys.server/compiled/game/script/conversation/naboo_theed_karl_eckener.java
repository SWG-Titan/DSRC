...
public class naboo_theed_karl_eckener extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (naboo_theed_karl_eckener_condition_completedDarkwalkerQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_37");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (naboo_theed_karl_eckener_condition_lastStepRescueQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_73");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (naboo_theed_karl_eckener_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "naboo_theed_karl_eckener", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
        ...
        if (naboo_theed_karl_eckener_condition_prisonerControlPanelTask(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_19");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (naboo_theed_karl_eckener_condition_onRescueQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_36");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (naboo_theed_karl_eckener_condition_talkToKarl(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 2 && naboo_theed_karl_eckener_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && naboo_theed_karl_eckener_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && naboo_theed_karl_eckener_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 8 && naboo_theed_karl_eckener_handleBranch8(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && naboo_theed_karl_eckener_handleBranch9(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
