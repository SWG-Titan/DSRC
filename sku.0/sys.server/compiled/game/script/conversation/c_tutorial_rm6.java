...
public class c_tutorial_rm6 extends script.base_script
{
    ...
    public boolean c_tutorial_rm6_condition_taskComplete(obj_id player, obj_id npc) throws InterruptedException
    {
        int questId = questGetQuestId("quest/c_newbie_hall_range01");
        int explainWeapons = groundquests.getTaskId(questId, "explainWeapons");
        return questIsTaskComplete(questId, explainWeapons, player);
    }
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (c_tutorial_rm6_condition_playerOnQuest(player, self))
        {
            doAnimationAction(self, "explain");
            c_tutorial_rm6_action_facePlayer(player, self);
            string_id message = new string_id(c_stringFile, "s_63");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (c_tutorial_rm6_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "c_tutorial_rm6", message, responses);
            }
            else 
            {
        ...
        if (c_tutorial_rm6_condition_taskComplete(player, self))
        {
        ...
        if (c_tutorial_rm6_condition_playerOnQuestStep05(player, self))
        {
        ...
        if (c_tutorial_rm6_condition_playerOnQuestStep0502(player, self))
        {
            doAnimationAction(self, "point_forward");
            c_tutorial_rm6_action_facePlayer(player, self);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && c_tutorial_rm6_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && c_tutorial_rm6_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && c_tutorial_rm6_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && c_tutorial_rm6_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && c_tutorial_rm6_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && c_tutorial_rm6_handleBranch9(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 11 && c_tutorial_rm6_handleBranch11(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
