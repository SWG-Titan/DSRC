...
public class station_corellia_mining extends base_script
{
    ...
    public void station_corellia_mining_action_sellResources(obj_id player, obj_id npc) throws InterruptedException
    {
        space_crafting.sellResourcesToSpaceStation(player, npc);
    }
    public int station_corellia_mining_handleBranch1(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
        if (response.equals("s_558"))
        {
            if (station_corellia_mining_condition__defaultCondition(player, npc))
            {
                station_corellia_mining_action_sellResources(player, npc);
    ...
    public int OnInitialize(obj_id self) throws InterruptedException
    {
        ...
        {
            detachScript(self, "conversation.station_corellia_mining");
        }
    ...
    {
        if (!conversationId.equals("station_corellia_mining"))
        {
            return SCRIPT_CONTINUE;
        }
