...
public class mtp_meatlumps_supplies_giver extends base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (mtp_meatlumps_supplies_giver_condition_returningQuestSuccess(player, self))
        {
            mtp_meatlumps_supplies_giver_action_sendSuppliesSuccessSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_4");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_meatlumps_supplies_giver_condition_returningQuestFailure(player, self))
        {
            mtp_meatlumps_supplies_giver_action_sendSuppliesFailureSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_19");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_meatlumps_supplies_giver_condition_alreadyHasQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_7");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_meatlumps_supplies_giver_condition_notYetReadyForAnother(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_9");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_meatlumps_supplies_giver_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "mtp_meatlumps_supplies_giver", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 5 && mtp_meatlumps_supplies_giver_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
