...
public class huff_darklighter extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (huff_darklighter_condition_returningFromSennex(player, self))
        {
            doAnimationAction(self, "udaman");
            huff_darklighter_action_signal_endHuffSennex(player, self);
            string_id message = new string_id(c_stringFile, "s_20");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (huff_darklighter_condition_workingOnHuffSennex(player, self))
        {
        ...
        if (huff_darklighter_condition_returningFromDera(player, self))
        {
            doAnimationAction(self, "bow");
            huff_darklighter_action_signal_endHuffDera(player, self);
            string_id message = new string_id(c_stringFile, "s_36");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (huff_darklighter_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "huff_darklighter", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
        ...
        if (huff_darklighter_condition_workingOnHuffDera(player, self))
        {
        ...
        if (huff_darklighter_condition_startWithHuff(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 3 && huff_darklighter_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && huff_darklighter_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
