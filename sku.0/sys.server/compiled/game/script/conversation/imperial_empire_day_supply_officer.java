...
public class imperial_empire_day_supply_officer extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (imperial_empire_day_supply_officer_condition_hasNotCraftedSupplies(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_17");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (imperial_empire_day_supply_officer_condition_hasImperialMissionActiveWithTask(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_5");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (imperial_empire_day_supply_officer_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "imperial_empire_day_supply_officer", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 2 && imperial_empire_day_supply_officer_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && imperial_empire_day_supply_officer_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
