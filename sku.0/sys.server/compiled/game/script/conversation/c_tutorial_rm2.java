...
public class c_tutorial_rm2 extends base_script
{
    ...
    public boolean c_tutorial_rm2_condition_playerOnQuest(obj_id player, obj_id npc) throws InterruptedException
    {
        ...
        return questIsQuestActive(questId, player) && !questIsTaskComplete(questId, explainInventory, player) && questIsTaskComplete(questId, explainRibbon, player);
    }
    public boolean c_tutorial_rm2_condition_taskComplete(obj_id player, obj_id npc) throws InterruptedException
    {
        int questId = questGetQuestId("quest/c_newbie_hall_01");
        int explainInventory = groundquests.getTaskId(questId, "explainInventory");
        return questIsTaskComplete(questId, explainInventory, player);
    }
    public boolean c_tutorial_rm2_condition_playerOnQuestStep01(obj_id player, obj_id npc) throws InterruptedException
    {
        int questId = questGetQuestId("quest/c_newbie_hall_01");
        int explainRibbon = groundquests.getTaskId(questId, "explainRibbon");
        return questIsQuestActive(questId, player) && !questIsTaskComplete(questId, explainRibbon, player);
    }
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (c_tutorial_rm2_condition_playerOnQuest(player, self))
        {
            doAnimationAction(self, "greet");
            c_tutorial_rm2_action_facePlayer(player, self);
            string_id message = new string_id(c_stringFile, "s_45");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (c_tutorial_rm2_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "c_tutorial_rm2", message, responses);
            }
            else 
            {
        ...
        if (c_tutorial_rm2_condition_taskComplete(player, self))
        {
        ...
        if (c_tutorial_rm2_condition_playerOnQuestStep01(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && c_tutorial_rm2_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && c_tutorial_rm2_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && c_tutorial_rm2_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && c_tutorial_rm2_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && c_tutorial_rm2_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && c_tutorial_rm2_handleBranch9(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 10 && c_tutorial_rm2_handleBranch10(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
