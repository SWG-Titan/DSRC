...
public class ep3_ghrag_traitor extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (ep3_ghrag_traitor_condition_isTooFar(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_87");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (ep3_ghrag_traitor_condition__defaultCondition(player, self))
        {
            ep3_ghrag_traitor_action_playCommSound(player, self);
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "ep3_ghrag_traitor", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 2 && ep3_ghrag_traitor_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && ep3_ghrag_traitor_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && ep3_ghrag_traitor_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 11 && ep3_ghrag_traitor_handleBranch11(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 12 && ep3_ghrag_traitor_handleBranch12(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
