...
public class itp_emperor_imp_tech extends base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (itp_emperor_imp_tech_condition_emperorTaskActive(player, self))
        {
            itp_emperor_imp_tech_action_emperorSignal(player, self);
            string_id message = new string_id(c_stringFile, "s_32");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (itp_emperor_imp_tech_condition__defaultCondition(player, self))
        {
