...
public class collections_fixbrokenspacekillcollections extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (collections_fixbrokenspacekillcollections_condition__defaultCondition(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "collections_fixbrokenspacekillcollections", message, responses);
            }
            else 
            {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && collections_fixbrokenspacekillcollections_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && collections_fixbrokenspacekillcollections_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && collections_fixbrokenspacekillcollections_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 9 && collections_fixbrokenspacekillcollections_handleBranch9(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 15 && collections_fixbrokenspacekillcollections_handleBranch15(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 21 && collections_fixbrokenspacekillcollections_handleBranch21(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 27 && collections_fixbrokenspacekillcollections_handleBranch27(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 33 && collections_fixbrokenspacekillcollections_handleBranch33(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
