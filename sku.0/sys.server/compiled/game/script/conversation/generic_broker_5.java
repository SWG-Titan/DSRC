...
public class generic_broker_5 extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (generic_broker_5_condition_isNonSmuggler(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_4");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (generic_broker_5_condition_isDoingMission(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_6");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (!generic_broker_5_condition_missionTierRequirement(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_8");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (generic_broker_5_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "generic_broker_5", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
        ...
        if (generic_broker_5_condition__defaultCondition(player, self))
        {
            ...
            if (generic_broker_5_condition_hasSomeContraband(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse3 = true;
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 3 && generic_broker_5_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && generic_broker_5_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && generic_broker_5_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 10 && generic_broker_5_handleBranch10(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 15 && generic_broker_5_handleBranch15(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
