...
public class junk_nathan_taike extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (junk_nathan_taike_condition__defaultCondition(player, self))
        {
            junk_nathan_taike_action_face_to(player, self);
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "junk_nathan_taike", message, responses);
            }
            else 
            {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && junk_nathan_taike_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && junk_nathan_taike_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && junk_nathan_taike_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && junk_nathan_taike_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
