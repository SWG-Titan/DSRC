...
public class npe_station_junk_dealer extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (npe_station_junk_dealer_condition__defaultCondition(player, self))
        {
            npe_station_junk_dealer_action_facePlayer(player, self);
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "npe_station_junk_dealer", message, responses);
            }
            else 
            {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && npe_station_junk_dealer_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 2 && npe_station_junk_dealer_handleBranch2(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
