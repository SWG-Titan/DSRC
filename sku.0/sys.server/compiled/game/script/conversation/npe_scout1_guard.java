...
public class npe_scout1_guard extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (npe_scout1_guard_condition_playerCompletedQuest(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_12");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (npe_scout1_guard_condition_playerCompletedQuestStep(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_5");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (npe_scout1_guard_condition_playerOnQuestStep(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_7");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (npe_scout1_guard_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "npe_scout1_guard", message, responses);
            }
            else 
            {
                chat.chat(self, player, message);
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 3 && npe_scout1_guard_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && npe_scout1_guard_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 5 && npe_scout1_guard_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && npe_scout1_guard_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
