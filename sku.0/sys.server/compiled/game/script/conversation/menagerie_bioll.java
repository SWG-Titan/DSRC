...
public class menagerie_bioll extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (menagerie_bioll_condition_needsNewQuestVar(player, self))
        {
            menagerie_bioll_action_FaceToAndAssignQuestVar(player, self);
            string_id message = new string_id(c_stringFile, "s_6");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (menagerie_bioll_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "menagerie_bioll", message, responses);
            }
            else 
            {
        ...
        if (menagerie_bioll_condition_hasVarBiollQuest(player, self))
        {
            ...
            if (hasResponse)
            {
                ...
                pp.actor.set(player);
                pp.target.set(self);
                pp.digitInteger = menagerie_bioll_tokenDI_varMenagerieQuest(player, self);
                npcStartConversation(player, self, "menagerie_bioll", null, pp, responses);
            }
            else 
            {
                prose_package pp = new prose_package();
        ...
        if (menagerie_bioll_condition_hasVarNonBiollQuest(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && menagerie_bioll_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && menagerie_bioll_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 5 && menagerie_bioll_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
