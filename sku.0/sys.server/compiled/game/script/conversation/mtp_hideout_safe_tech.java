...
public class mtp_hideout_safe_tech extends script.base_script
{
    ...
    public boolean mtp_hideout_safe_tech_condition_abortedRoriTalus(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public boolean mtp_hideout_safe_tech_condition_roriTalusActiveComplete(obj_id player, obj_id npc) throws InterruptedException
    {
    ...
    public int mtp_hideout_safe_tech_handleBranch7(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int mtp_hideout_safe_tech_handleBranch11(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int mtp_hideout_safe_tech_handleBranch12(obj_id player, obj_id npc, string_id response) throws InterruptedException
    {
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (mtp_hideout_safe_tech_condition_abortedRoriTalus(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_49");
            int numberOfResponses = 0;
            boolean hasResponse = false;
            boolean hasResponse0 = false;
            if (mtp_hideout_safe_tech_condition__defaultCondition(player, self))
            {
                ++numberOfResponses;
                hasResponse = true;
                hasResponse0 = true;
            ...
            if (hasResponse)
            {
                ...
                npcStartConversation(player, self, "mtp_hideout_safe_tech", message, responses);
            }
            else 
            {
        ...
        if (mtp_hideout_safe_tech_condition_isEligible(player, self))
        {
        ...
        if (mtp_hideout_safe_tech_condition_isReturningSuccess(player, self))
        {
        ...
        if (mtp_hideout_safe_tech_condition_isActive(player, self))
        {
            string_id message = new string_id(c_stringFile, "s_44");
            chat.chat(self, player, message);
            return SCRIPT_CONTINUE;
        }
        if (mtp_hideout_safe_tech_condition_hasCompleted(player, self))
        {
    ...
    public int OnNpcConversationResponse(obj_id self, String conversationId, obj_id player, string_id response) throws InterruptedException
    {
        ...
        if (branchId == 1 && mtp_hideout_safe_tech_handleBranch1(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 3 && mtp_hideout_safe_tech_handleBranch3(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 4 && mtp_hideout_safe_tech_handleBranch4(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 5 && mtp_hideout_safe_tech_handleBranch5(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 6 && mtp_hideout_safe_tech_handleBranch6(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 7 && mtp_hideout_safe_tech_handleBranch7(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 8 && mtp_hideout_safe_tech_handleBranch8(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 10 && mtp_hideout_safe_tech_handleBranch10(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 11 && mtp_hideout_safe_tech_handleBranch11(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
        if (branchId == 12 && mtp_hideout_safe_tech_handleBranch12(player, self, response) == SCRIPT_CONTINUE)
        {
            return SCRIPT_CONTINUE;
        }
