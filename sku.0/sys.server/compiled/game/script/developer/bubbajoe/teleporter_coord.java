...
@Origin: script.developer.bubbajoe.teleporter_coord
@Author: BubbaJoeX
@Purpose: This script is used to teleport players to a custom set location via the radial menu.
@Note: Avoid using mustafar or kashyyyk_main as they may have offset coordinates, unless you can do the math.
...
public class teleporter_coord extends script.base_script
{
    public static final String STF = "city/city";

    public teleporter_coord()
    {
    }
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            LOG("ethereal", "[Teleporter]: Teleporting " + getPlayerFullName(player) + " to " + planet + " at " + x + ", " + y + ", " + z);
        }
        if (item == menu_info_types.SERVER_MENU1)
        {
            ...
            String ycoord = getStringObjVar(self, "teleport.y");
            String zcoord = getStringObjVar(self, "teleport.z");
            broadcast(player, "How to: Use radial submenus to set Scene and XZY Coordinates to check in order to teleport.");
