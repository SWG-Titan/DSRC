...
public class buff_group_click_item extends script.base_script
{
    ...
    public static final string_id MUST_BE_GROUPED = new string_id("base_player", "must_be_grouped");
    ...
    public int OnObjectMenuRequest(obj_id self, obj_id player, menu_info mi) throws InterruptedException
    {
        if (canManipulate(player, self, true, true, 15, true))
        {
            if (utils.isNestedWithinAPlayer(self))
            {
                ...
                {
                    mi.addRootMenu(menu_info_types.ITEM_USE, new string_id("Seek Knowledge"));
                }
            }
        }
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        ...
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            {
                CustomerServiceLog("buff", "buff_group_click_item object self: " + self + " Name: " + getName(self) + " had an invalid static item name. Buff object is bailing out early as a result.");
                return SCRIPT_CONTINUE;
            }
            ...
            if (itemData == null)
            {
                CustomerServiceLog("buff", "buff_group_click_item object self: " + self + " Name: " + getName(self) + " had invalid item data and as a result the buff object is bailing out early.");
                return SCRIPT_CONTINUE;
            }
                    ...
                    if (buff.canApplyBuff(player, buffName))
                    {
                        ...
                        if (isValidId(myGroup))
                        {
                            ...
                            for (obj_id member : members)
                            {
                                ...
                                LOG("ethereal", "[Entertainer Book] object self: " + self + " Static Item Name: " + itemName + " providing buff: " + buffName + " being used by player: " + player + " Name: " + getName(player) + " of player level: " + playerLevel + " and buff was applied to group member: " + member + " Name: " + getName(member) + " of player level: " + getLevel(member));
                        ...
                        {
                            broadcast(player, "You must be in a group to use this item.");
                            return SCRIPT_CONTINUE;
                        }
                        if (getCount(self) > 0)
                        {
