...
public class generic_gift_box_extended extends base_script
{
    ...
    public generic_gift_box_extended()
    {
    }

    public int OnObjectMenuRequest(obj_id self, obj_id player, menu_info mi) throws InterruptedException
    {
        mi.addRootMenu(menu_info_types.ITEM_USE, new string_id("Claim Items"));
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        if (item == menu_info_types.ITEM_USE)
        {
            ...
            if (!boxValid)
            {
                broadcast(player, "The system has encountered an error. Please contact a CSR. [Reference ID: " + getSelf() + "]");
                return SCRIPT_CONTINUE;
            }
            ...
            if (source.equals(SOURCE_TABLE))
            {
                ...
                {
                    boxValid = false;
                    broadcast(player, "The system is unable to find the items to grant. Please contact a CSR. [Reference ID: " + getSelf() + "]");
                    return SCRIPT_CONTINUE;
                }
            }
            ...
            if (boxValid)
            {
                broadcast(player, "You have claimed your items.");
                destroyObject(self);
            }
            else
            {
    ...
    private boolean createGiftBoxItemsFromTable(String tableName, String mode, int selections, obj_id player) throws InterruptedException
    {
        ...
        ArrayList<String> itemsToCreate = new ArrayList<>();
        ArrayList<Integer> itemCountToCreate = new ArrayList<>();
        int createdCount = 0;
    ...
    {
        LOG("ethereal", "[WBB]: Created box and attached script item.gift_boxes.generic_gift_box");
        return SCRIPT_CONTINUE;
    }

    public int OnInitialize(obj_id self)
    {
        LOG("ethereal", "[WBB]: Initialized box, This item " + (hasScript(self, "item.gift_boxes.generic_gift_box") ? "has the proper script" : "does not have the proper script") + " attached.");
        return SCRIPT_CONTINUE;
    }
}