...
public class imperial_recruitment_sign extends base_script
{
    public imperial_recruitment_sign()
    {
    }

    // EMPIRE DAY - REBEL DEFACEMENT OF IMPERIAL RECRUITMENT SIGN
    ...
    public static final string_id SID_MNU_DRESS = new string_id(EMPIREDAY, "destroy_recruitment_sign");
    ...
    public int OnObjectMenuRequest(obj_id self, obj_id player, menu_info mi) throws InterruptedException
    {
        ...
        if (!(getTemplateName(self)).equals(holiday.REBEL_NONPLAYER_RECRUITMENT_KIOSK))
        {
            return SCRIPT_CONTINUE;
        }
        ...
        if (!hasCompletedCollectionSlotPrereq(player, holiday.REBEL_VANDAL_COUNTER_SLOT))
        {
            return SCRIPT_CONTINUE;
        }
        if (!buff.hasBuff(player, holiday.BUFF_REB_EMPIREDAY_VANDAL_COMBATANT) && !buff.hasBuff(player, holiday.BUFF_REB_EMPIREDAY_VANDAL_SF))
        {
            return SCRIPT_CONTINUE;
        }
    ...
    public int handleObjectSwapTimer(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        detachScript(player, sui.COUNTDOWNTIMER_PLAYER_SCRIPT);
        if (buff.hasBuff(player, holiday.BUFF_REB_EMPIREDAY_VANDAL_SF) && !factions.isDeclared(player))
        {
            sendSystemMessage(player, holiday.SID_NOT_SF);
            return SCRIPT_CONTINUE;
        }
        if (!holiday.replaceEventObject(self, player, holiday.REBEL_PLAYER_DEFACED_KIOSK))
        {
            return SCRIPT_CONTINUE;
        }
        else 
        {
            ...
            if (buff.hasBuff(player, holiday.BUFF_REB_EMPIREDAY_VANDAL_COMBATANT))
            {
                parentBuffName = holiday.BUFF_REB_EMPIREDAY_VANDAL_COMBATANT;
            }
            else if (buff.hasBuff(player, holiday.BUFF_REB_EMPIREDAY_VANDAL_SF))
            {
                parentBuffName = holiday.BUFF_REB_EMPIREDAY_VANDAL_SF;
            }
            if (parentBuffName.equals(""))
            {
                return SCRIPT_CONTINUE;
            }
            if (!holiday.rewardEmpireDayPlayer(player, holiday.REBEL_PLAYER, parentBuffName, holiday.BUFF_REBEL_PLAYER_VANDAL_COUNTER, holiday.REBEL_VANDAL_COUNTER_SLOT, holiday.EMPIRE_DAY_VANDAL_LOCKED_OUT))
            {
                return SCRIPT_CONTINUE;
            }
        }
