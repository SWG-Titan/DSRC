...
public class event_tool extends script.base_script
{
        ...
        if (st.hasMoreTokens())
        {
            ...
            if (command.equals("grantCommunityBadge"))
            {
                ...
                if (st.hasMoreTokens())
                {
                    ...
                    LOG("events", "[EventTool] 'grantCommunityBadge' used by " + getPlayerName(self) + ":" + self + ". Awarding badge " + badgeName + " to " + getPlayerName(player) + ":" + player + " .", null);
            ...
            if (command.equals("grantAllCommunityBadges"))
            {
                ...
                LOG("events", "[EventTool] 'grantAllCommunityBadges' used by " + getPlayerName(self) + ":" + self + ". Awarding all community badges to " + getPlayerName(player) + ":" + player + " .", null);
            ...
            if (command.equals("grantCommunityBadgeArea"))
            {
                ...
                if (st.hasMoreTokens())
                {
                    ...
                    LOG("events", "[EventTool] 'grantCommunityBadgeArea' used by " + getPlayerName(self) + ":" + self + ". Awarding badge " + badgeName + " to " + badgeCount + " people.", null);
                }
                else
                {
    ...
    public int eventVaderChoke(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventVaderChoke used by [" + getName(self) + ":" + self + "] on [" + getName(myTarget) + ":" + myTarget, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventClearStates(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventClearStates used by " + selfName + ":" + self, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventBuff(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        String selfName = getName(self);
        LOG("events", "[EventTool] /eventBuff used by " + selfName + ":" + self + ". Params: " + params, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventBuffTarget(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventBuffTarget used by " + selfName + ":" + self + " on target " + targetName + ":" + myTarget + ". Params: " + params, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventBuffArea(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        if (params == null || params.isEmpty())
        {
            broadcast(self, "[Syntax] /eventBuffArea <buff name> <player/npc/all> <range> <optional duration> <optional value>. Refer to /sys.shared/compiled/game/datatables/buff/buff.tab for proper buff names.");
            return SCRIPT_CONTINUE;
        }
        ...
        {
            broadcast(self, "Incorrect number of arguments. [Syntax] /eventBuffArea <buff name> <player/npc/all> <range> <optional duration> <optional value>. Refer /sys.shared/compiled/game/datatables/buff/buff.tab for proper buff names.");
            return SCRIPT_CONTINUE;
        }
        ...
        float range = utils.stringToFloat(rangeStr);

        if (!isValidCommandType(commandType))
        {
            broadcast(self, "Target for eventBuffArea must be player, npc or all.[Syntax] /eventBuffArea <buff name> <player/npc/all> <range> <optional duration> <optional value>.");
            return SCRIPT_CONTINUE;
        }

        if (!isValidRange(range))
        {
            sendSystemMessage(self, "Invalid range for eventBuffArea. Range must be between 1 and 512.", null);
            return SCRIPT_CONTINUE;
        }
        ...
        if (numArgs > 3)
        {
            duration = utils.stringToInt(st.nextToken());
        }
        if (numArgs > 4)
        {
            value = utils.stringToInt(st.nextToken());
        }

        applyBuffsInRange(self, commandType, buffName, range, duration, value, numArgs);

        logEventUsage(self, params);
        return SCRIPT_CONTINUE;
    }

    private boolean isValidCommandType(String commandType)
    {
        return commandType.equals("player") || commandType.equals("npc") || commandType.equals("all");
    }

    private boolean isValidRange(float range)
    {
        return range >= 1 && range <= 512;
    }

    private void applyBuffsInRange(obj_id self, String commandType, String buffName, float range, float duration, float value, int numArgs) throws InterruptedException
    {
        obj_id[] thingsInRange = null;

        if (commandType.equals("player"))
        {
            thingsInRange = getPlayerCreaturesInRange(self, range);
        }
        else if (commandType.equals("npc"))
        {
            thingsInRange = getNPCsInRange(self, range);
        }
        else if (commandType.equals("all"))
        {
            thingsInRange = getCreaturesInRange(self, range);
        }
    ...
    private void logEventUsage(obj_id self, String params)
    {
        String selfName = getName(self);
        location here = getLocation(self);
        LOG("events", "[EventTool] /eventBuffArea used by " + selfName + ":" + self + " at location " + here + ". Params: " + params, null);
    }
    ...
    public int eventTefArea(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        String selfName = getName(self);
        location here = getLocation(self);
        LOG("events", "[EventTool] /eventTefArea used by " + selfName + ":" + self + " at location " + here, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventTefTarget(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        String selfName = getName(self);
        String targetName = getName(myTarget);
        LOG("events", "[EventTool] /eventTefTarget used by " + selfName + ":" + self + " on target " + targetName + ":" + myTarget, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventTefClear(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        String selfName = getName(self);
        LOG("events", "[EventTool] /eventTefClear used by " + selfName + ":" + self, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int grantBadgeArea(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        sendSystemMessage(self, "A total of " + badgeCount + " badges were awarded.", null);
        LOG("events", "[EventTool] /grantBadgeArea used by " + getName(self) + ":" + self + ". Awarding badge #" + badgeNumber + " to " + badgeCount + " people.", null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int maxForcePower(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /maxForcePower used by " + getName(self) + ":" + self, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventDamage(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventDamage used by " + getName(self) + ":" + self + " on target " + getName(myTarget) + ":" + myTarget + " for " + damage + " damage.", null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventMoveToMe(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventMoveToMe used by " + getName(self) + ":" + self + " on target " + getName(myTarget) + ":" + target + " from location " + here + " to location " + getLocation(self), null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventUndoMoveToMe(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventUndoMoveToMe used by " + getName(self) + ":" + self + " on target " + getName(lastTarget) + ":" + lastTarget + " from location " + currentTargetLocation + " to location " + lastLocation, null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventWeapon(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventWeapon used by " + getName(self) + ":" + self + " to create " + weaponTemplate + " at " + powerMult + "% of maximum strength.", null);
        return SCRIPT_CONTINUE;
    }
    ...
    public int eventStorePet(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        LOG("events", "[EventTool] /eventStorePet used by " + getName(self) + ":" + self + " to store pet " + getName(myTarget) + ":" + target, null);
        return SCRIPT_CONTINUE;
    }
