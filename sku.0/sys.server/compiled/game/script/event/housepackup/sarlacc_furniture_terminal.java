...
public class sarlacc_furniture_terminal extends base_script
{
    ...
    public static final string_id SID_GOD_ALLOW_PLAYER_SIFT = new string_id("sarlacc_minigame", "god_allow_player_sift");
    public static final string_id SID_SOUND_ON = new string_id("sarlacc_minigame", "sarlacc_sound_on");
    public static final string_id SID_SOUND_OFF = new string_id("sarlacc_minigame", "sarlacc_sound_off");
    public static final string_id SID_MENU_SOUND_OFF = new string_id("sarlacc_minigame", "sarlacc_sound_menu_off");
    public static final string_id SID_MENU_SOUND_ON = new string_id("sarlacc_minigame", "sarlacc_sound_menu_on");
    ...
    public int OnDestroy(obj_id self) throws InterruptedException
    {
        ...
        customerSvcLogVerbose(self, "Sarlacc: " + self + " is being destroyed.");
    ...
    public int OnInitialize(obj_id self) throws InterruptedException
    {
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " has been initialized. Attempting to update sarlacc data.");
            if (!updateSarlaccPet(self))
            {
                CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " could not be updated properly or the update attempt was too soon.");
            }
        }
        messageTo(self, "checkTriggerVolume", null, 2, false);
        return SCRIPT_CONTINUE;
    ...
    public boolean setUpFirstTime(obj_id self) throws InterruptedException
    {
        ...
        customerSvcLogVerbose(self, "Sarlacc: " + self + " has been set up for the very first time.");
        return true;
    }
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        ...
        if (hasObjVar(self, house_pet.SARLACC_LAST_FED))
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " has been feed previously and used via radial option. Attempting to update sarlacc data.");
            if (!updateSarlaccPet(self))
            {
                CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " could not be updated properly or the update attempt was too soon.");
            }
        ...
        if (item == menu_info_types.SERVER_MENU1)
        {
            ...
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " has selected the feed option via radial.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " has a reward for the owner: " + player + " and the owner has selected the reward option via radial.");
            utils.removeScriptVar(self, house_pet.SARLACC_AVOID_REPEATED_UPDATES);
            if (!updateSarlaccPet(self))
            {
                CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " could not be updated properly or the update attempt was too soon.");
        ...
        else if (item == menu_info_types.SERVER_MENU3)
        {
            ...
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is being fed by CS or God Player: " + player + ".");
            utils.removeScriptVar(self, house_pet.SARLACC_AVOID_REPEATED_UPDATES);
    ...
    public int handleSarlaccEatSui(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (idx < 0)
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " selected an invalid option in the list of edibles.");
            cleanupSui(player);
            return SCRIPT_CONTINUE;
        }
        if (bp == sui.BP_CANCEL)
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " selected cancel.");
            cleanupSui(player);
            return SCRIPT_CONTINUE;
        }
        else 
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " selected a valid edible item.");
            obj_id[] junkFood = utils.getObjIdBatchScriptVar(player, house_pet.EDIBLES_IDS);
            if ((junkFood == null) || (junkFood.length == 0))
            {
                CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " selected something but the list of items came back corrupt or invalid.");
                cleanupSui(player);
                return SCRIPT_CONTINUE;
            }
            ...
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " has selected item: " + junkFood[idx] + " name: " + getName(junkFood[idx]) + " from the UI list.");
    ...
    public int verifySarlaccEatSui(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (bp == sui.BP_CANCEL)
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " has cancelled the feeding via the verificaiton UI.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " somehow lost their item variable needed to feeding the sarlacc.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " is hungry and the owner: " + player + " somehow deleted the item he was about to feed to the sarlacc.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player (" + getPlayerName(player) + " OID: " + player + ") FAILED TO FEED sarlacc: " + self + " item (" + getName(junkFood) + " OID: " + junkFood + ").");
        }
        cleanupSui(player);
        return SCRIPT_CONTINUE;
    }
    ...
    public boolean updateSarlaccPet(obj_id sarlacc) throws InterruptedException
    {
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " avoiding repeated calls to this function. Bailing early.");
            return false;
        }
        else 
        {
        ...
        customerSvcLogVerbose(sarlacc, "updateSarlaccPet - RECEIVED MESSAGE TO UPDATE SARLACC: " + sarlacc + " at: time: " + getCalendarTime());
        if (!hasObjVar(sarlacc, house_pet.SARLACC_LAST_FED))
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has never been fed. Exiting updateSarlaccPet.");
            blog("updateSarlaccPet - no last feed time");
            return false;
        }
        else 
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has been either been fed previously or has a valid empty variable. Continuing debug procedures.");
        }
        if (!hasObjVar(sarlacc, house_pet.SARLACC_CURRENT_PHASE))
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has NO CURRENT PHASE VAR. Exiting updateSarlaccPet.");
            blog("updateSarlaccPet - no phase for sarlacc");
            return false;
        }
        else 
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has a current phase, continuing debug procedures.");
        }
        if (!hasObjVar(sarlacc, house_pet.SARLACC_FEEDING_ITERATION))
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has NO FEEDING ITERATION VAR. Exiting updateSarlaccPet.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " is currently dead.");
            String collectionCol = getSarlaccCollectionColumn(sarlacc);
            if (collectionCol == null || collectionCol.equals(""))
            {
                CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " reward is about to be issued but a correct column was not returned. Setting reward to pick from generic_collectible");
                blog("updateSarlaccPet - getSarlaccCollectionColumn FAILED!");
                collectionCol = "generic_collectible";
            }
            else 
            {
                CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " reward is about to be issued. The random column returned was:" + collectionCol);
            }
            blog("updateSarlaccPet - getSarlaccCollectionColumn: " + collectionCol);
            customerSvcLogVerbose(sarlacc, "updateSarlaccPet - Collection column received. We are going to reset the Feeding iteration, Feeding list and set the Sarlacc to reward the player. Before we do this, lets get verbose data. ");
        ...
        CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " is not dead. We are continuing the updated.");
        if (currentPhase < house_pet.SARLACC_FED)
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " current phase was -1.");
        ...
        if (hasObjVar(sarlacc, house_pet.SARLACC_VERSION_VAR))
        {
            ...
            {
                CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has no version number so the sarlacc is being stamped for the first time. The sarlacc also gets the free chance variable (or it resets this variable).");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has no version number so the sarlacc is being stamped for the first time. ");
            setObjVar(sarlacc, house_pet.SARLACC_VERSION_VAR, house_pet.SARLACC_VERSION);
        }
        if (currentGameTime < (lastFed + updateWeek))
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has received an update before the request too soon. Bailing out.");
            blog("updateSarlaccPet - Last Fed Timer: " + (lastFed + updateWeek) + " currentGameTime: " + currentGameTime);
            return false;
        }
        CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - It has been at least a week since the Sarlacc: " + sarlacc + " has had food.");
        if (currentGameTime < (lastFed + (updateWeek * 2)))
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " is supposed to be hungry but has time before being Very Hungry. We are going to check the current phase.");
            if (currentPhase != house_pet.SARLACC_HUNGRY)
            {
                CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " had a bugged current phase of: " + currentPhase + " so we will correct that now.");
                setObjVar(sarlacc, house_pet.SARLACC_CURRENT_PHASE, house_pet.SARLACC_HUNGRY);
            }
            else 
            {
                CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " had a bugged current phase is Hungry which is correct.");
            }
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " is currently hungry but still has time before moving to very hungry. Bailing out.");
            return false;
        }
        if (currentGameTime >= (lastFed + updateWeek) && currentGameTime < (lastFed + (updateWeek * 2)))
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " was phase " + currentPhase + " and is now being set to HUNGRY.");
            setObjVar(sarlacc, house_pet.SARLACC_CURRENT_PHASE, house_pet.SARLACC_HUNGRY);
            return true;
        }
        CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has been neglected by owner. Checking for penalties.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - It has been at least 2 weeks since the Sarlacc: " + sarlacc + " has had food but the player hasn't used up their free chance yet.  We'll take care of this when the player feed this sarlacc.");
            setObjVar(sarlacc, house_pet.SARLACC_PENALTY_VAR, true);
            setObjVar(sarlacc, house_pet.SARLACC_CURRENT_PHASE, house_pet.SARLACC_HUNGRY);
            return false;
        }
        CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " has been neglected by owner and the free chance has been used.");
        if (currentGameTime >= (lastFed + (updateWeek * 3)))
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - It has been at least 2 weeks since the Sarlacc: " + sarlacc + " has had food, but less than 3 weeks.");
            currentPhase += 3;
        }
        ...
        if (currentPhase >= house_pet.SARLACC_DEATH)
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " was neglected and is currently dead (Starving).");
            currentPhase = house_pet.SARLACC_DEATH;
        }
        if (currentPhase >= house_pet.SARLACC_STARVING)
        {
            CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " is about to be set to have data removed because phase is: " + currentPhase);
            if (!setSarlaccStarvingOrDead(sarlacc, currentPhase))
            {
                CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " could not remove data. This is a big error that needs to be reported to SWG Development. Phase is: " + currentPhase);
                return false;
            }
        }
        return true;
    ...
    public boolean getEdibleContents(obj_id player, obj_id sarlacc) throws InterruptedException
    {
        ...
        else 
        {
            if (getIntObjVar(sarlacc, house_pet.SARLACC_CURRENT_PHASE) < house_pet.SARLACC_HUNGRY)
            {
                CustomerServiceLog("sarlacc_minigame: ", "updateSarlaccPet - Sarlacc: " + sarlacc + " was fed, even though the Sarlacc was not hungry because the CS Representative: " + player + " did so intentionally while in godmode");
    ...
    public boolean eatJunkItem(obj_id self, obj_id player, obj_id item) throws InterruptedException
    {
        ...
        CustomerServiceLog("sarlacc_minigame: ", "eatJunkItem - Player ( OID: " + player + ") has fed sarlacc item (" + item + "). Processing");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " has NO previous feeding list. Creating a new one.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " has A previous feeding list. Getting data.");
            String[] feedingList = getStringArrayObjVar(self, house_pet.SARLACC_FEED_ARRAY);
            if (feedingList == null || feedingList.length <= 0)
            {
                CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " previous feeding list was bugged or null. Creating a new list.");
            ...
            {
                CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " previous feeding list was " + feedingList.length + " in length. Adding another item to this list.");
                ...
                setObjVar(self, house_pet.SARLACC_FEED_ARRAY, newList);
                CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " new feeding list NOW has " + newList.length + " items.");
            }
        }
        int currentIteration = getIntObjVar(self, house_pet.SARLACC_FEEDING_ITERATION);
        CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " Current Feeding Iteration: " + currentIteration);
        currentIteration++;
        CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " Current Feeding Iteration is now: " + currentIteration);
        ...
        CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " Current Phase right after being fed is: " + getIntObjVar(self, house_pet.SARLACC_CURRENT_PHASE));
        setObjVar(self, house_pet.SARLACC_CURRENT_PHASE, house_pet.SARLACC_FED);
        CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " Current Phase now is: " + getIntObjVar(self, house_pet.SARLACC_CURRENT_PHASE));
        if (!hasObjVar(self, house_pet.NO_SARLACC_SOUND))
        {
        ...
        CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " Last Fed WAS: " + getIntObjVar(self, house_pet.SARLACC_LAST_FED));
        setObjVar(self, house_pet.SARLACC_LAST_FED, getCalendarTime());
        CustomerServiceLog("sarlacc_minigame: ", "Sarlacc: " + self + " Last Fed NOW: " + getIntObjVar(self, house_pet.SARLACC_LAST_FED));
    ...
    public boolean validateItemForSarlacc(obj_id player, obj_id item) throws InterruptedException
    {
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player: " + player + " is attempting to give a sarlacc item: " + item + " but that item is not in their inventory");
            sendSystemMessage(player, SID_ITEM_NOT_INVENTORY);
            return false;
        }
        if (smuggler.hasItemsInContainer(item))
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player: " + player + " is attempting to give a sarlacc item: " + item + " but that item is not in their ROOT inventory.  It may be nested in another container.");
            sendSystemMessage(player, SID_ITEM_NOT_INVENTORY);
            return false;
        }
        if (utils.isEquipped(item))
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player: " + player + " is attempting to give a sarlacc item: " + item + " but that item is equipped.");
            sendSystemMessage(player, SID_ITEM_EQUIPPED);
            return false;
        }
        if (hasObjVar(item, "quest_item"))
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player: " + player + " is attempting to give a sarlacc item: " + item + " but that item is a quest item.");
    ...
    public String getSarlaccCollectionColumn(obj_id self) throws InterruptedException
    {
        ...
        if (!hasObjVar(self, house_pet.SARLACC_FEED_ARRAY))
        {
            CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " could not select a reward collection column because there war no list of items fed on the sarlacc.");
            return null;
        }
        blog("getSarlaccCollectionColumn - array of feed exists");
        CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " is selecting a reward collection column based on the list of items fed.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " found a corrupt list of items fed and could not continue.");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " has eaten the following: " + listData);
        }
        HashSet allEdibles = new HashSet();
        for (int i = 0; i < foodList.length - 1; i++)
        {
            if (foodList[i] == null || foodList[i].equals(""))
            {
                continue;
            }
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " could not put all fed items into a hashset and failed out of the Collection Column function!");
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " failed in the Collection Column function when attempting to make a hashset and array!");
        ...
        if (templateArray.length == 1)
        {
            ...
            if (lootRow < 0)
            {
                CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " could not find the table row needed for the item fed and returned the default random column.");
            ...
            {
                CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " could not find the collectible_cols row needed for the item fed and returned the default random column.");
                return "generic_collectible";
            }
            else 
            {
                ...
                {
                    CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " had corrupt data and returned default generic_collectible.");
                    return "generic_collectible";
                }
                else if (possibleCols.length == 1)
                {
                    CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " had length of 1 and is returning: " + possibleCols[0]);
                    return possibleCols[0];
                }
                else 
                {
                    int randNum = rand(0, possibleCols.length - 1);
                    CustomerServiceLog("sarlacc_minigame: ", "getSarlaccCollectionColumn - Sarlacc: " + self + " had length of 1 and is returning a random item: " + possibleCols[randNum]);
    ...
    public boolean getPlayerReward(obj_id owner, obj_id self) throws InterruptedException
    {
        ...
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player (" + getPlayerName(owner) + " OID: " + owner + ") could not receive reward for feeding sarlacc. Sarlacc reward column objvar was not on the sarlacc item. THIS MIGHT BE DUE TO THE PLAYER ATTEMPTING TO GET MULTIPLE REWARDS via quick radial select exploit.");
            return false;
        }
        String rewardCol = getStringObjVar(self, house_pet.SARLACC_REWARD_COLUMN);
        if (rewardCol == null || rewardCol.equals(""))
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player (" + getPlayerName(owner) + " OID: " + owner + ") could not receive reward for properly feeding sarlacc. Sarlacc reward column objvar was found but invalid.");
            return false;
        }
        ...
        CustomerServiceLog("sarlacc_minigame: ", "Player (" + getPlayerName(owner) + " OID: " + owner + ") is about to receive Sarlacc reward. Using collection library for reward. Check the CollectionLootChannel logs for specifics or search for the player OID to get full details.");
        if (!collection.getRandomCollectionItemShowLootBox(owner, playerInventory, house_pet.SARLACC_LOOT_TABLE, rewardCol))
        {
            CustomerServiceLog("sarlacc_minigame: ", "Player (" + getPlayerName(owner) + " OID: " + owner + ") FAILED TO ISSUE PLAYER REWARD. Sarlacc: " + self + ". This probably failed in the collection library being used to grant the player a reward item.");
            blog("rewardPlayer - severe issue with getRandomCollectionItem for col: " + rewardCol);
        }
        CustomerServiceLog("sarlacc_minigame: ", "Player (" + getPlayerName(owner) + " OID: " + owner + ") has received reward. Check the CollectionLootChannel logs for specifics or search for the player OID to get full details.");
    ...
    public boolean setSarlaccStarvingOrDead(obj_id sarlacc, int currentPhase) throws InterruptedException
    {
        ...
        customerSvcLogVerbose(sarlacc, "updateSarlaccPet - About to Wipe Sarlacc data and set feeding iteration to 0. This is the last point in which you will see previous data. Current time: " + getCalendarTime());
        setObjVar(sarlacc, house_pet.SARLACC_FEEDING_ITERATION, 0);
        removeObjVar(sarlacc, house_pet.SARLACC_FEED_ARRAY);
    ...
    public boolean customerSvcLogVerbose(obj_id sarlacc, String msg) throws InterruptedException
    {
        ...
        {
            logData += " No valid sarlacc owner.";
        }
        ...
        {
            logData += " Sarlacc has never been fed.";
        }
        if (hasObjVar(sarlacc, house_pet.SARLACC_FEEDING_ITERATION))
        {
            int currentIteration = getIntObjVar(sarlacc, house_pet.SARLACC_FEEDING_ITERATION);
        ...
        if (hasObjVar(sarlacc, house_pet.SARLACC_VERSION_VAR))
        {
            ...
            logData += " Sarlacc Version Number: " + sarlaccVer + ".";
        }
        else 
        {
            logData += " No Sarlacc Version Number for this sarlacc.";
        }
        ...
        {
            logData += " No Sarlacc Free Chance Variable for this sarlacc.";
        }
        if (hasObjVar(sarlacc, house_pet.SARLACC_PENALTY_VAR))
        {
            logData += " Sarlacc has PENALTY VARIABLE.";
        }
        else 
        {
