...
public class player_nb extends script.base_script
{
    ...
    public static void updatePlayerStats(obj_id self) throws InterruptedException
    {
        ...
        try
        {
            ...
            String checkQuery = "SELECT COUNT(*) FROM player_stats WHERE player_id = ?";
            stmt = conn.prepareStatement(checkQuery);
            stmt.setInt(1, Integer.parseInt(self.toString()));

            rs = stmt.executeQuery();

            // If the player exists, perform an UPDATE
            if (rs.next() && rs.getInt(1) > 0)
            {
                ...
                stmt.setInt(1, getTotalMoney(self));
                stmt.setString(2, getLocation(self).toReadableFormat(true));
                stmt.setString(3, factions.getFaction(self));
                stmt.setString(4, "active");
                stmt.setString(5, getPlayerFullName(self));
                stmt.setInt(6, getPlayerBirthDate(self));
                stmt.setInt(7, getFilledVolume(utils.getInventoryContainer(self)));
                stmt.setInt(8, getLevel(self));
                stmt.setInt(9, Integer.parseInt(self.toString()));
            ...
            {
                // If the player does not exist, perform an INSERT
                String insertQuery = "INSERT INTO player_stats "
                        + "(player_id, total_money, location, faction, status, name, birthdate, inventory_free, plevel, created_at, updated_at) "
                        + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)";
                stmt = conn.prepareStatement(insertQuery);
                stmt.setInt(1, Integer.parseInt(self.toString()));
                stmt.setInt(2, getTotalMoney(self));
                stmt.setString(3, getLocation(self).toReadableFormat(true));
                stmt.setString(4, factions.getFaction(self));
                stmt.setString(5, "active");
                stmt.setString(6, getPlayerFullName(self));
                stmt.setInt(7, (getPlayerBirthDate(self)));
                stmt.setInt(8, getFilledVolume(utils.getInventoryContainer(self)));
                stmt.setInt(9, getLevel(self));

                // Execute the insert
                stmt.executeUpdate();
            }

            // Commit the transaction (optional depending on your connection settings)
            conn.commit();
    ...
    private void grantAnniversaryPresent(obj_id self)
    {
    ...
    public void grantChristmasPresent(obj_id self)
    {
    ...
    public int createServerWelcomePage(obj_id self, String welcome, String title)
    {
    ...
    private void fillCandyBowl(obj_id self)
    {
    ...
    public int getTokenCalculations(obj_id self)
    {
    ...
    public int cmdInviteGroupArea(obj_id self, obj_id target, String params, float defaultTime)
    {
    ...
    public int cmdAnnounceTrade(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        ...
        else
        {
            ...
            {
                or_player.sendTradeUpdateToDiscord(self, params, "Galactic Trading Association: " + getPlayerFullName(self), false, false, false);
    ...
    public int cmdCityWarn(obj_id self, obj_id target, String params, float defaultTime)
    {
    ...
    public int sellInvJunk(obj_id self, dictionary params) throws InterruptedException
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
        ...
        if (bp == sui.BP_CANCEL)
        {
            broadcast(self, "You have chosen not to sell all of your loot.");
            return SCRIPT_CONTINUE;
        }
        else
        {
            ...
            {
                if (hasObjVar(item, smuggler.JUNK_DEALKER_NO_SALE_FLAG))
                {
                    continue;
                }
                smuggler.sellJunkItemFromUI(self, item, false, false);
            }
            int totalProfits = getIntObjVar(self, "ui.lastPayout");
            debugConsoleMsg(self, "You have sold a total of " + totalProfits + " credits worth of loot.");
            playClientEffectObj(self, "clienteffect/utinni.cef", self, "");
            setObjVar(self, "ui.sellLootLast", getCalendarTime());
        }
        return SCRIPT_CONTINUE;
    }

    public int cmdVehicleMechanic(obj_id self, obj_id target, String params, float defaultTime)
    {
    ...
    private void startStatSyncLoop(obj_id self)
    {
        messageTo(self, "handleStatSync", null, 300.0f, false);
    }

    public int handleStatSync(obj_id self, dictionary params)
    {
        try
        {
            if (!hasObjVar(self, "stat_sync"))
            {
                setObjVar(self, "stat_sync", true);
                startStatSyncLoop(self);
            }
            else
            {
                // Optionally log or handle the case where ObjVar already exists
                LOG("stat_sync", "ObjVar exists, skipping sync loop.");
                return SCRIPT_CONTINUE;
            }

            // Update player stats regardless
